 ...
 */
public class SVDRMSETopicTimePlus_topic_weighting_new extends SVD {
			...
			for(int k = 0; k < Config.K; ++k){
				...
				for (Integer t : nu.keySet())
					Y[t][k] = Y[t][k] * (1 - Config.lambda1 * Config.eta / Math.sqrt(nu.size()))
							+ Config.eta * err * it[k]/ Math.sqrt(nu.size());
				for (Integer t : ni.keySet())
					Y[t][k] = Y[t][k] * (1 - Config.lambda1 * Config.eta / Math.sqrt(ni.size()))
							+ Config.eta * err * ut[k]/ Math.sqrt(ni.size());
				for (Integer t : recent_nu.keySet())
					T[t][k] = T[t][k] * (1 - Config.lambda1 * Config.eta / Math.sqrt(recent_nu.size()))
							+ Config.eta * err * it[k]/ Math.sqrt(recent_nu.size());
				for (Integer t : recent_ni.keySet())
					T[t][k] = T[t][k] * (1 - Config.lambda1 * Config.eta / Math.sqrt(recent_ni.size()))
							+ Config.eta * err * ut[k]/ Math.sqrt(recent_ni.size());
			}
